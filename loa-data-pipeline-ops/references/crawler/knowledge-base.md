...
生产日志默认采用人工门禁：由人先登录 BytePlus 控制台，再使用获批密钥进入 `LOA-crawler-prod`。AI 可以根据代码提供只读命令、目录和结果判读，但不索取、保存或代持密钥，也不默认从本机直连生产服务器。
...
- 2026-08-24 已确认本机现有 SSH 配置可连接 Crawler 生产主机，但配置存在不构成持续访问授权，也不记录 SSH 用户或密钥路径。每次生产访问仍需按具体目标取得明确授权。
