...
现有 workflow 不保存旧 JAR，也没有 rollback input。按照生产发布/版本回滚只能通过 GitHub Actions 执行的控制面边界，本次采用重新运行发布前最后一次已知成功生产 Action 的回滚方式；该选择不依赖某台操作机是否保存 SSH 私钥。
...
- 2026-08-21 当时批准的生产日志访问方式是人工登录 BytePlus 后回传脱敏结果。该历史边界已于 2026-08-25 被 skill 的“既有批准通道可由 AI 执行严格只读访问”规则取代；仍不得索取或保存密钥，也不得执行任何状态变更。
- 当前 skill 的受控只读访问规则是后续操作边界，不会补齐本次 2026-08-19 实验当时没有取得的 JAR 校验值、PID、活跃连接和服务器日志证据。
